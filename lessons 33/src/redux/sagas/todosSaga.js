...
const fetchSwapiApi = async (url) => {
    const response = await fetch(url);
    if (!response.ok) throw new Error('Failed to fetch Swapi data');
    return response.json();
  };
  ...
  function* fetchSwapiSaga(action) {
    try {
      ...
      yield put({ type: 'swapi/fetchDataSuccess', payload: data });
    } catch (error) {
      yield put({ type: 'swapi/fetchDataFailure', payload: error.message });
    }
  }
...
export function* todoSaga() {
  ...
  yield takeEvery('swapi/fetchData', fetchSwapiSaga);
}